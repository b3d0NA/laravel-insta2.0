...
<div class="container mx-auto lg:px-32 p-10">
    <div class="create-post-wrapper mx-auto flex justify-center pt-3">
        <div class="box-border h-auto lg:w-6/12 w-full p-4 border-2 border-opacity-30 rounded-md">
{{--            <h2 class="text-gray-800 text-2xl py-4">Make post</h2>--}}
            ...
            <form action="{{route("post.create")}}" method="post" enctype="multipart/form-data">
                @csrf
                <textarea name="body" id="postBody" class="w-full bg-white rounded-md h-32 p-3" placeholder="Type your posts text here">{{old("body")}}</textarea>
                <input type="file" name="image">
                <input type="submit" name="submit" value="Post" class="cursor-pointer text-white block py-2 px-6 my-5 rounded-md bg-blue-500">
            </form>
        </div>
        </div>
    </div>
            ...
            <div class="post-wrapper lg:mx-auto flex justify-center pt-3">
                <div class="box-border h-auto md:w-10/12 sm:w-screen lg:w-6/12 p-4 border-2 border-opacity-30 rounded-md">
                    <div class="post-top pb-5">
                        <a href="{{route("user.profile", ["user" => $post->user])}}">
    ...
    @else
        <div class="post-wrapper mx-auto flex justify-center pt-3">
            <div class="box-border h-auto w-6/12 p-4 border-2 border-opacity-30 rounded-md">
                ...
                <h1 class="text-3xl text-center text-gray-600 py-9">There is no post available at this moment! Be the first one.</h1>
            </div>
        </div>
